...
/**
 * Checks if the user is valid based on expiration and traffic limit.
 * @param {object} userData - User data object.
 * @returns {boolean} - True if valid, false otherwise.
 */
async function checkValid(userData) {
  const dateValid = await checkExpiration(userData.exp_date, userData.exp_time);
  const trafficValid = userData.traffic_limit === null || userData.traffic_used < userData.traffic_limit;
  return dateValid && trafficValid;
}
...
/**
 * Updates traffic usage in the database.
        ...
        if (userRouteMatch && request.method === 'PUT') {
            ...
            try {
                ...
                await env.DB.prepare(`UPDATE users SET ${setClauses} WHERE uuid = ?`)
                    .bind(...binds).run();
                await env.USER_KV.put(`user:${uuid}`, JSON.stringify({ exp_date: expDate, exp_time: expTime, traffic_limit, traffic_used: reset_traffic ? 0 : undefined }));
...
export default {
  async fetch(request, env, ctx) {
    ...
    const handleSubscription = async (core) => {
      ...
      if (!userData || !(await checkValid(userData))) {
        return new Response('Invalid or expired or over limit user', { status: 403 });
      }
    ...
    if (isValidUUID(path)) {
      ...
      return handleConfigPage(path, url.hostname, cfg.proxyAddress, userData.exp_date, userData.exp_time, userData.traffic_limit, userData.traffic_used);
    }
...
async function ProtocolOverWSHandler(request, config, env) {
  ...
  readableWebSocketStream
    .pipeTo(
      new WritableStream({
        async write(chunk, controller) {
          ...
          uuid = stringify(new Uint8Array(chunk.slice(1, 17)));
          ...
          if (isUDP) {
            if (portRemote === 53) {
              ...
              udpStreamWriter = (chunk) => {
                inBytes += chunk.byteLength;
                return dnsPipeline.write(chunk);
              };
    ...
    .finally(async () => {
      await updateUsage(env, uuid, inBytes + outBytes);
    });
...
async function ProcessProtocolHeader(protocolBuffer, env) {
  ...
  if (!userData || !(await checkValid(userData))) {
    return { hasError: true, message: 'invalid or expired or over limit user' };
  }
...
async function RemoteSocketToWS(remoteSocket, webSocket, protocolResponseHeader, retry, log) {
  ...
  let vlessResponseHeader = protocolResponseHeader;
  try {
      ...
      new WritableStream({
        async write(chunk, controller) {
          ...
          if (vlessResponseHeader) {
            dataToSend = await new Blob([vlessResponseHeader, chunk]).arrayBuffer();
...
function handleConfigPage(userID, hostName, proxyAddress, expDate, expTime, trafficLimit, trafficUsed) {
  const html = generateBeautifulConfigPage(userID, hostName, proxyAddress, expDate, expTime, trafficLimit, trafficUsed);
...
function generateBeautifulConfigPage(userID, hostName, proxyAddress, expDate = '', expTime = '', trafficLimit = null, trafficUsed = 0) {
  ...
  <body data-proxy-ip="${proxyAddress}">
    ${getPageHTML(singleXrayConfig, singleSingboxConfig, clientUrls, subXrayUrl, subSbUrl).replace('', expirationBlock + usageBlock)}
