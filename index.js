/**
* Cloudflare Worker VLESS Proxy - Ultimate Combined Edition
*
* [Final Merged & Corrected Version by Gemini]
*
* This script combines the stability of Script 1's connection core with the
* advanced features of Script 2. 
*
* [GEMINI FINAL FIX - 2025-10-24]:
* - FIXED: The critical nested template literal syntax error.
* - CAUSE: The `adminPanelHTML` and `generateBeautifulConfigPage` variables 
* were defined using template literals (`...`), and the inline <script> 
* tags within them *also* used template literals (`fetch(`${api_base}`...`)`),
* which is a syntax error in JavaScript.
* - SOLUTION: All inline JavaScript within those two HTML strings has been
* rewritten to use standard string concatenation (+) instead of 
* nested template literals, completely resolving the syntax error.
*
* FEATURES:
* - STABLE CONNECTION CORE: Uses the SOCKS5-aware, robust connection handler
* (ProtocolOverWSHandler, HandleTCPOutBound, etc.) from Script 1.
* - ADVANCED ADMIN PANEL: Uses the admin panel (HTML, JS, API) from Script 2,
* including D1 database, stats, and DATA LIMIT (GB/MB) management.
* - ADVANCED USER PAGE: Uses the smart user config page from Script 2,
* with network info and DATA USAGE bar.
* - TRAFFIC TRACKING & DATA LIMITS: Logic from Script 2 injected into Script 1's core.
* Counts TOTAL (Upload + Download) traffic.
* - MANAGEMENT API: Includes the /api/v1/ management API from Script 2.
* - PERFORMANCE: Uses ctx.waitUntil for non-blocking operations.
*
* SETUP:
* 1. Create D1 Database (DB binding): Run SQL:
...
* );
* 2. Create KV Namespace (USER_KV binding).
* 3. Set Secrets: ADMIN_KEY (password), API_TOKEN (optional).
* 4. Set Variables: ADMIN_PATH (optional, default /admin), UUID (optional),
* PROXYIP, SCAMALYTICS_USERNAME (optional), SCAMALYTICS_API_KEY (optional),
* ROOT_PROXY_URL (optional), SOCKS5 (optional), SOCKS5_RELAY (optional).
* 5. Add to wrangler.toml: [compatibility_flags] sockets = true
*/

import { connect } from 'cloudflare:sockets';

// --- Configuration (from Script 2) ---
const Config = {
  userID: 'd342d11e-d424-4583-b36e-524ab1f0afa4', // Default fallback
  proxyIPs: [], // Set PROXYIP in your env variables
...
async function getUserData(env, uuid, ctx) {
  ...
  if (!env.DB || !env.USER_KV) { console.error("D1 or KV bindings missing."); return null; }
  const cacheKey = `user:${uuid}`;
  try {
    const cachedData = await env.USER_KV.get(cacheKey, 'json');
    if (cachedData && cachedData.uuid) return cachedData;
  } catch (e) { console.error(`KV parse error for ${uuid}`, e); }

  const userFromDb = await env.DB.prepare("SELECT * FROM users WHERE uuid = ?").bind(uuid).first();
  if (!userFromDb) return null;

  const cacheOp = env.USER_KV.put(cacheKey, JSON.stringify(userFromDb), { expirationTtl: 3600 });
  if (ctx) ctx.waitUntil(cacheOp); else await cacheOp;
  return userFromDb;
}

// --- Admin Panel & API (All from Script 2, Syntax Error Corrected) ---
...
<body>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // ***** START: CORRECTED API OBJECT (using + for concatenation) *****
            const api = {
            ...
            async function refreshData() {
                try {
                    ...
                    renderStats(stats);
                    renderUsers(users);
                } catch (error) { console.error('Failed to refresh data:', error); showToast(error.message, true); }
            }
            ...
            document.getElementById('createUserForm').addEventListener('submit', async e => {
                ...
                if (!uuid || !expiryDate || !expiryTime) { showToast('UUID, Expiry Date, and Expiry Time are required.', true); return; }
                const { utcDate, utcTime } = localToUTC(expiryDate, expiryTime);
                const userData = { uuid, exp_date: utcDate, exp_time: utcTime, data_limit: getLimitInBytes('dataLimitValue', 'dataLimitUnit'), notes: document.getElementById('notes').value };
                try { await api.post('/users', userData); showToast('User created successfully!'); e.target.reset(); document.getElementById('uuid').value = crypto.randomUUID(); setDefaultExpiry(); refreshData(); } catch (error) { showToast(error.message, true); }
            });
            ...
            document.getElementById('userList').addEventListener('click', e => {
                ...
                if (button.classList.contains('btn-copy-uuid')) { navigator.clipboard.writeText(uuid); showToast('UUID copied'); }
                    ...
                    if (confirm('Delete user ' + uuid.substring(0,8) + '...?')) {
                        // [GEMINI FIX]
                        api.delete('/users/' + uuid).then(() => { showToast('User deleted'); refreshData(); }).catch(err => showToast(err.message, true));
                    }
                }
            });
            ...
            document.getElementById('editUserForm').addEventListener('submit', async e => {
                ...
                const updatedData = { exp_date: utcDate, exp_time: utcTime, data_limit: getLimitInBytes('editDataLimitValue', 'editDataLimitUnit'), notes: document.getElementById('editNotes').value, reset_traffic: document.getElementById('resetTraffic').checked };
                try { 
                    // [GEMINI FIX]
                    await api.put('/users/' + uuid, updatedData); 
                    showToast('User updated'); editModal.classList.remove('show'); refreshData(); 
            ...
            document.getElementById('uuid').value = crypto.randomUUID(); setDefaultExpiry(); refreshData();
        });
    </script>
</body>
</html>`;
// ***** END: CORRECTED adminPanelHTML *****
...
function handleApiError(e, context = '') {
  ...
  if (e instanceof Error) errorMsg = e.message;
    ...
    if (e.cause?.message) errorMsg += `: ${e.cause.message}`;
  }
  if (errorMsg.includes('UNIQUE constraint failed')) errorMsg = 'UUID already exists.';
...
async function handleAdminRequest(request, env, cfg, ctx) {
  ...
  if (!env.ADMIN_KEY) return new Response(JSON.stringify({ error: 'Admin panel not configured.' }), { status: 503, headers: jsonHeader });
  if (!env.DB || !env.USER_KV) return new Response(JSON.stringify({ error: 'D1 or KV bindings missing.' }), { status: 503, headers: jsonHeader });
  ...
  if (pathname.startsWith(apiBasePath)) {
    ...
    if (apiPath === '/stats' && request.method === 'GET') { 
      try {
        const stats = await env.DB.prepare( "SELECT COUNT(*) as totalUsers, SUM(CASE WHEN DATETIME(expiration_date || 'T' || expiration_time || 'Z') > CURRENT_TIMESTAMP THEN 1 ELSE 0 END) as activeUsers, SUM(CASE WHEN DATETIME(expiration_date || 'T' || expiration_time || 'Z') <= CURRENT_TIMESTAMP THEN 1 ELSE 0 END) as expiredUsers, SUM(data_usage) as totalTraffic FROM users").first();
        return new Response(JSON.stringify({ totalUsers: stats?.totalUsers||0, activeUsers: stats?.activeUsers||0, expiredUsers: stats?.expiredUsers||0, totalTraffic: stats?.totalTraffic||0 }), { status: 200, headers: jsonHeader });
      } catch (e) { return handleApiError(e, 'GET /stats'); }
    }
    if (apiPath === '/users' && request.method === 'GET') { 
      ...
      } catch (e) { return handleApiError(e, 'GET /users'); }
    }
    if (apiPath === '/users' && request.method === 'POST') { 
      try {
        ...
        if (!uuid || !exp_date || !exp_time || !isValidUUID(uuid)) throw new Error('Invalid/missing fields.');
        await env.DB.prepare("INSERT INTO users (uuid, expiration_date, expiration_time, notes, data_limit) VALUES (?, ?, ?, ?, ?)").bind(uuid, exp_date, exp_time, notes_to_bind, data_limit_to_bind).run();
        return new Response(JSON.stringify({ success: true, uuid }), { status: 201, headers: jsonHeader });
      } catch (e) { return handleApiError(e, 'POST /users'); }
    }
    ...
    if (userRouteMatch) {
      ...
      if (request.method === 'PUT') { 
        ...
        } catch (e) { return handleApiError(e, `PUT /users/${uuid}`); }
      }
      if (request.method === 'DELETE') { 
        ...
        } catch (e) { return handleApiError(e, `DELETE /users/${uuid}`); }
      }
    }
  ...
  if (pathname === cfg.adminPath) { 
    if (request.method === 'POST') {
      ...
      if (!loginCsrfToken || !formCsrfToken || loginCsrfToken !== formCsrfToken) {
        const errorHtml = adminLoginHTML.replace('</form>', '</form><p class="error">Invalid session. Please try again.</p>');
      ...
      if (formData.get('password') === env.ADMIN_KEY) {
        const sessionToken = crypto.randomUUID(); const csrfToken = crypto.randomUUID();
        ctx.waitUntil(env.USER_KV.put(`admin_session:${sessionToken}`, JSON.stringify({ csrfToken }), { expirationTtl: 86400 }));
        const headers = new Headers({ 'Location': cfg.adminPath, 'Set-Cookie': `auth_token=${sessionToken}; HttpOnly; Secure; Path=${cfg.adminPath}; Max-Age=86400; SameSite=Strict` });
        headers.append('Set-Cookie', `login_csrf_token=; Path=${cfg.adminPath}; Expires=Thu, 01 Jan 1970 00:00:00 GMT`);
    ...
    if (request.method === 'GET') {
      ...
      if (isAdmin) {
        ...
        const loginHtmlWithCsrf = adminLoginHTML.replace('</form>', `<input type="hidden" name="csrf_token" value="${loginCsrfToken}"></form>`);
        const headers = new Headers({ 'Content-Type': 'text/html;charset=utf-8' });
...
async function handleManagementAPI(request, env, cfg, ctx) { 
  const jsonHeader = { 'Content-Type': 'application/json' };
  if (!cfg.apiToken || request.headers.get('Authorization') !== `Bearer ${cfg.apiToken}`) return new Response(JSON.stringify({ error: 'Forbidden' }), { status: 403, headers: jsonHeader });
  if (!env.DB || !env.USER_KV) return new Response(JSON.stringify({ error: 'Service partially configured.' }), { status: 503, headers: jsonHeader });
  const url = new URL(request.url);
  const userRouteMatch = url.pathname.match(/^\/api\/v1\/users\/([a-f0-9-]+)$/);
  if (url.pathname === '/api/v1/users' && request.method === 'GET') {
    try { const { results } = await env.DB.prepare("SELECT * FROM users").all(); return new Response(JSON.stringify(results ?? []), { status: 200, headers: jsonHeader }); } catch (e) { return handleApiError(e, 'GET /api/v1/users'); }
  }
  if (url.pathname === '/api/v1/users' && request.method === 'POST') {
    try { const body = await request.json(); const { uuid, exp_date, exp_time } = body; if (!uuid||!exp_date||!exp_time||!isValidUUID(uuid)) throw new Error('Invalid/missing fields.'); const notes_to_bind = body.hasOwnProperty('notes')?(body.notes||null):null; let data_limit=0; if(typeof body.data_limit_gb==='number')data_limit=body.data_limit_gb*1073741824; else if(typeof body.data_limit_mb==='number')data_limit=body.data_limit_mb*1048576; await env.DB.prepare("INSERT INTO users (uuid, expiration_date, expiration_time, notes, data_limit) VALUES (?, ?, ?, ?, ?)").bind(uuid,exp_date,exp_time,notes_to_bind,data_limit).run(); const user=await getUserData(env,uuid,ctx); return new Response(JSON.stringify(user), { status: 201, headers: jsonHeader }); } catch (e) { return handleApiError(e, 'POST /api/v1/users'); }
  }
  if (userRouteMatch) {
    const uuid = userRouteMatch[1];
    if (request.method === 'GET') {
      try { const user=await getUserData(env,uuid,ctx); if(!user)return new Response(JSON.stringify({error:'User not found'}),{status:404,headers:jsonHeader}); return new Response(JSON.stringify(user),{status:200,headers:jsonHeader}); } catch (e) { return handleApiError(e, `GET /api/v1/users/${uuid}`); }
    }
    if (request.method === 'DELETE') {
      try { await env.DB.prepare("DELETE FROM users WHERE uuid = ?").bind(uuid).run(); ctx.waitUntil(env.USER_KV.delete(`user:${uuid}`)); return new Response(null,{status:204}); } catch (e) { return handleApiError(e, `DELETE /api/v1/users/${uuid}`); }
    }
    if (request.method === 'PUT') {
      try { const user=await getUserData(env,uuid,ctx); if(!user)return new Response(JSON.stringify({error:'User not found'}),{status:404,headers:jsonHeader}); const body=await request.json(); const exp_date=body.exp_date||user.expiration_date; const exp_time=body.exp_time||user.expiration_time; const notes_to_bind=body.hasOwnProperty('notes')?(body.notes||null):user.notes; let data_limit=user.data_limit; if(typeof body.data_limit_gb==='number')data_limit=body.data_limit_gb*1073741824; else if(typeof body.data_limit_mb==='number')data_limit=body.data_limit_mb*1048576; const sql=`UPDATE users SET expiration_date = ?, expiration_time = ?, notes = ?, data_limit = ? ${body.reset_traffic?', data_usage = 0':''} WHERE uuid = ?`; await env.DB.prepare(sql).bind(exp_date,exp_time,notes_to_bind,data_limit,uuid).run(); ctx.waitUntil(env.USER_KV.delete(`user:${uuid}`)); const updatedUser=await getUserData(env,uuid,ctx); return new Response(JSON.stringify(updatedUser),{status:200,headers:jsonHeader}); } catch (e) { return handleApiError(e, `PUT /api/v1/users/${uuid}`); }
    }
  }
  return new Response(JSON.stringify({ error: 'API route not found' }), { status: 404, headers: jsonHeader });
}
...
function generateRandomPath(length = 12, query = '') { 
  const chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789'; let result = ''; for (let i = 0; i < length; i++) result += chars.charAt(Math.floor(Math.random() * chars.length)); return `/${result}${query ? `?${query}` : ''}`;
}
const CORE_PRESETS = { 
...
function createVlessLink({ userID, address, port, host, path, security, sni, fp, alpn, extra = {}, name }) { 
  const params = new URLSearchParams({ type: 'ws', host, path, }); if (security) params.set('security', security); if (sni) params.set('sni', sni); if (fp) params.set('fp', fp); if (alpn) params.set('alpn', alpn); for (const [k, v] of Object.entries(extra)) params.set(k, v); return `vless://${userID}@${address}:${port}?${params.toString()}#${encodeURIComponent(name)}`;
}
function buildLink({ core, proto, userID, hostName, address, port, tag }) { 
  const p = CORE_PRESETS[core][proto]; return createVlessLink({ userID, address, port, host: hostName, path: p.path(), security: p.security, sni: p.security === 'tls' ? hostName : undefined, fp: p.fp, alpn: p.alpn, extra: p.extra, name: makeName(tag, proto), });
}
const pick = (arr) => arr[Math.floor(Math.random() * arr.length)];
async function handleIpSubscription(core, userID, hostName) { 
  const mainDomains = [ hostName, 'creativecommons.org', 'www.speedtest.net', 'sky.rethinkdns.com', 'cfip.1323123.xyz', 'go.inmobi.com', 'www.visa.com', 'cf.090227.xyz', 'cdnjs.com', 'zula.ir', ]; const httpsPorts = [443, 8443, 2053, 2083, 2087, 2096]; const httpPorts = [80, 8080, 8880, 2052, 2082, 2086, 2095]; let links = []; const isPagesDeployment = hostName.endsWith('.pages.dev'); mainDomains.forEach((domain, i) => { links.push( buildLink({ core, proto: 'tls', userID, hostName, address: domain, port: pick(httpsPorts), tag: `D${i+1}` }) ); if (!isPagesDeployment) links.push( buildLink({ core, proto: 'tcp', userID, hostName, address: domain, port: pick(httpPorts), tag: `D${i+1}` }) ); }); try { const r = await fetch('https://raw.githubusercontent.com/NiREvil/vless/refs/heads/main/Cloudflare-IPs.json'); if (r.ok) { const json = await r.json(); const ips = [...(json.ipv4 ?? []), ...(json.ipv6 ?? [])].slice(0, 20).map(x => x.ip); ips.forEach((ip, i) => { const formattedAddress = ip.includes(':') ? `[${ip}]` : ip; links.push( buildLink({ core, proto: 'tls', userID, hostName, address: formattedAddress, port: pick(httpsPorts), tag: `IP${i+1}` }) ); if (!isPagesDeployment) links.push( buildLink({ core, proto: 'tcp', userID, hostName, address: formattedAddress, port: pick(httpPorts), tag: `IP${i+1}` }) ); }); } } catch (e) { console.error('Fetch IP list failed', e); } return new Response(btoa(links.join('\n')), { headers: { 'Content-Type': 'text/plain;charset=utf-8' }, });
}

// --- Smart Config Page Generation (from Script 2) ---

async function getIPGeoInfo(ip) { 
    if (!ip) return null; try { const response = await fetch(`https://ip-api.com/json/${ip}?fields=status,message,country,city,isp,query,as`); if (!response.ok) return null; const data = await response.json(); if (data.status === 'success') return { ip: data.query, country: data.country || 'Unknown', city: data.city || 'Unknown', isp: data.isp || data.as || 'Unknown' }; return null; } catch (e) { console.error(`IP info error for ${ip}:`, e); return null; }
}
async function getIPRiskScore(ip, cfg) { 
    if (!ip || !cfg.scamalytics.username || !cfg.scamalytics.apiKey) return "N/A (Not Configured)"; try { const url = `${cfg.scamalytics.baseUrl}${cfg.scamalytics.username}/?key=${cfg.scamalytics.apiKey}&ip=${ip}`; const response = await fetch(url); if (!response.ok) return "N/A (API Error)"; const data = await response.json(); if (data.status === 'ok') { const riskText = data.risk.charAt(0).toUpperCase() + data.risk.slice(1); return `${data.score} - ${riskText}`; } return `Error: ${data.error_message || 'API error'}`; } catch (e) { return "N/A (Fetch Error)"; }
}
async function handleNetworkInfoRequest(request, env, cfg, ctx) { 
    const userIP = request.headers.get('CF-Connecting-IP'); let proxyIPInfo = await env.USER_KV?.get('proxy_ip_info', 'json'); if (!proxyIPInfo) { let determinedIP = cfg.proxyIP;
        if (!determinedIP) {
            try {
                const ipResponse = await fetch('https://api.ipify.org?format=json');
                if (ipResponse.ok) determinedIP = (await ipResponse.json()).ip;
            } catch (e2) {
                console.error('Proxy IP fallback failed:', e2);
            }
        }
    ...
    return new Response(JSON.stringify({ proxy: proxyIPInfo, user: userInfo }), { headers: { 'Content-Type': 'application/json' } });
}
async function handleConfigPage(userID, hostName, proxyAddress, userData, userIP, env, cfg, ctx) { 
    const { expiration_date, expiration_time, data_usage, data_limit } = userData; const html = generateBeautifulConfigPage(userID, hostName, expiration_date, expiration_time, data_usage, data_limit); return new Response(html, { headers: { 'Content-Type': 'text/html; charset=utf-8' } });
}

// ***** START: CORRECTED generateBeautifulConfigPage *****
// [GEMINI FIX]: Rewrote the inline <script> to use standard string concatenation (+)
// instead of nested template literals ( `${...}` ) to avoid syntax errors.
function generateBeautifulConfigPage(userID, hostName, expDate, expTime, dataUsage, dataLimit) {
    const subXrayUrl = `https://${hostName}/xray/${userID}`; const subSbUrl = `https://${hostName}/sb/${userID}`; const clashMetaUrl = `clash://install-config?url=${encodeURIComponent(subSbUrl)}`; const expTimeSeconds = expTime.includes(':') && expTime.split(':').length === 2 ? `${expTime}:00` : expTime; const utcTimestamp = `${expDate}T${expTimeSeconds.split('.')[0]}Z`; const isUserExpired = isExpired(expDate, expTime); const hasDataLimit = dataLimit > 0; const dataLimitReached = hasDataLimit && (dataUsage >= dataLimit); let statusMessage = "Checking..."; let statusColorClass = "status-active-text"; if (isUserExpired) { statusMessage = "Subscription Expired"; statusColorClass = "status-expired-text"; } else if (dataLimitReached) { statusMessage = "Data Limit Reached"; statusColorClass = "status-expired-text"; } const trafficPercent = hasDataLimit ? Math.min(100, (dataUsage / dataLimit * 100)) : 0;
    
    // [GEMINI FIX] All inner template literals in the <script> tag are now replaced with string concatenation.
    const html = `<!doctype html><html lang="en"><head><meta charset="UTF-8" /><meta name="viewport" content="width=device-width, initial-scale=1.0" /><title>VLESS Proxy Configuration</title><script src="https://cdnjs.cloudflare.com/ajax/libs/qrcodejs/1.0.0/qrcode.min.js"></script><style>:root{--bg-main:#121212;--bg-card:#1E1E1E;--bg-inner:#2f2f2f;--border-color:#333;--text-primary:#E0E0E0;--text-secondary:#B0B0B0;--accent:#BB86FC;--accent-hover:#D1C4E9;--status-active:#03DAC6;--status-expired:#CF6679;--network-bg:#212121;--network-border:#444}body{margin:0;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Helvetica,Arial,sans-serif;background-color:var(--bg-main);color:var(--text-primary);padding:20px}.container{max-width:900px;margin:auto}.header{text-align:center;margin-bottom:24px}.header h1{font-size:2em;margin-bottom:8px}.header p{color:var(--text-secondary)}.info-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(320px,1fr));gap:20px;margin-bottom:20px}.info-card{background:var(--bg-card);border-radius:12px;position:relative;overflow:hidden;border:1px solid var(--border-color)}.info-card.rainbow-border::before{content:'';position:absolute;top:-50%;left:-50%;width:200%;height:200%;background:conic-gradient(from 180deg at 50% 50%,#CF6679,#BB86FC,#03DAC6,#CF6679);animation:spin 4s linear infinite;z-index:1}.info-card-content{background:var(--bg-card);padding:20px;border-radius:10px;position:relative;z-index:2;margin:2px}.info-title{font-size:1.25em;text-align:center;margin:0 0 16px;font-weight:500}.info-relative-time{text-align:center;font-size:1.4em;font-weight:600;margin-bottom:16px}.status-active-text{color:var(--status-active)}.status-expired-text{color:var(--status-expired)}.info-time-grid{display:grid;gap:8px;font-size:.9em;text-align:center;color:var(--text-secondary)}.data-usage-text{font-size:1.4em!important;font-weight:600;text-align:center;color:var(--text-primary);margin-bottom:16px}.traffic-bar-container{height:8px;background-color:var(--bg-inner);border-radius:4px;overflow:hidden}.traffic-bar{height:100%;background:linear-gradient(90deg,var(--accent) 0%,var(--status-active) 100%);border-radius:4px;transition:width .5s ease-out}.network-info-wrapper{background:var(--bg-card);border-radius:12px;padding:24px;margin-bottom:24px;border:1px solid var(--border-color)}.network-info-header{display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;padding-bottom:16px;border-bottom:1px solid var(--border-color)}.network-info-header h2{margin:0;font-size:1.4rem}.network-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:20px}.network-card{background:var(--network-bg);border:1px solid var(--network-border);border-radius:8px;padding:16px}.network-title{font-size:1.1em;margin-top:0;margin-bottom:12px;border-bottom:1px solid var(--network-border);padding-bottom:8px;color:var(--status-active)}.network-info-grid{display:grid;gap:8px;font-size:.9em}.network-info-grid strong{color:var(--text-secondary);font-weight:400;display:inline-block;min-width:90px}.network-info-grid span{color:var(--text-primary);font-weight:500}.skeleton{display:inline-block;width:120px;height:1em;background-color:var(--bg-inner);border-radius:4px}.config-card{background:var(--bg-card);border-radius:12px;padding:24px;margin-bottom:24px;border:1px solid var(--border-color)}.config-title{display:flex;justify-content:space-between;align-items:center;font-size:1.4rem;margin-bottom:20px;padding-bottom:16px;border-bottom:1px solid var(--border-color)}.button,.client-btn{display:inline-flex;align-items:center;justify-content:center;gap:8px;padding:10px 16px;border-radius:8px;font-size:14px;font-weight:500;cursor:pointer;border:1px solid var(--border-color);background-color:var(--bg-inner);color:var(--text-primary);text-decoration:none;transition:all .2s}.button:hover{background-color:#3f3f3f}.client-buttons{display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:12px}.client-btn{width:100%;box-sizing:border-box;background-color:var(--accent);color:#121212;border:none}.client-btn:hover{background-color:var(--accent-hover)}.qr-container{display:none;margin-top:20px;background:#fff;padding:16px;border-radius:8px;max-width:288px;margin-left:auto;margin-right:auto}@keyframes spin{100%{transform:rotate(360deg)}}@media (max-width:768px){body{padding:10px}.info-grid,.network-grid{grid-template-columns:1fr}.network-info-header{flex-direction:column;align-items:flex-start;gap:10px}.network-info-header button{width:100%}}</style></head><body><div class="container"><div class="header"><h1>VLESS Proxy Configuration</h1><p>Copy the configuration or import directly into your client</p></div><div class="network-info-wrapper"><div class="network-info-header"><h2>Network Information</h2><button class="button" id="refresh-network-btn">Refresh</button></div><div id="network-info-grid" class="network-grid"><div class="network-card"><h3 class="network-title">Proxy Server</h3><div class="network-info-grid"><div><strong>IP Address:</strong> <span id="proxy-ip"><span class="skeleton"></span></span></div><div><strong>Location:</strong> <span id="proxy-location"><span class="skeleton"></span></span></div><div><strong>ISP:</strong> <span id="proxy-isp"><span class="skeleton"></span></span></div></div></div><div class="network-card"><h3 class="network-title">Your Connection</h3><div class="network-info-grid"><div><strong>IP Address:</strong> <span id="user-ip"><span class="skeleton"></span></span></div><div><strong>Location:</strong> <span id="user-location"><span class="skeleton"></span></span></div><div><strong>ISP:</strong> <span id="user-isp"><span class="skeleton"></span></span></div><div><strong>Risk Score:</strong> <span id="user-risk"><span class="skeleton"></span></span></div></div></div></div></div><div class="info-grid"><div class="info-card rainbow-border"><div class="info-card-content"><h2 class="info-title">Expiration Date</h2><div id="expiration-relative" class="info-relative-time ${statusColorClass}">${statusMessage}</div><div class="info-time-grid" id="expiration-display" data-utc-time="${utcTimestamp}"><div><strong>Your Local Time:</strong> <span id="local-time">--</span></div><div><strong>Tehran Time:</strong> <span id="tehran-time">--</span></div><div><strong>Universal Time:</strong> <span id="utc-time">--</span></div></div></div></div><div class="info-card"><div class="info-card-content"><h2 class="info-title">Data Usage</h2><div class="data-usage-text" id="data-usage-display" data-usage="${dataUsage}" data-limit="${dataLimit}">Loading...</div><div class="traffic-bar-container"><div class="traffic-bar" style="width:${trafficPercent}%"></div></div></div></div></div><div class="config-card"><div class="config-title"><span>Xray Subscription</span><button id="copy-xray-sub-btn" class="button" data-clipboard-text="${subXrayUrl}">Copy Link</button></div><div class="client-buttons"><a href="v2rayng://install-config?url=${encodeURIComponent(subXrayUrl)}" class="client-btn">V2rayNG / Universal</a><a href="shadowrocket://add/sub?url=${encodeURIComponent(subXrayUrl)}&name=${encodeURIComponent(hostName)}" class="client-btn">Shadowrocket</a><a href="stash://install-config?url=${encodeURIComponent(subXrayUrl)}" class="client-btn">Stash (VLESS)</a><button class="client-btn" onclick="toggleQR('xray', '${subXrayUrl}')">Show QR Code</button></div><div id="qr-xray-container" class="qr-container"><div id="qr-xray"></div></div></div><div class="config-card"><div class="config-title"><span>Sing-Box / Clash Subscription</span><button id="copy-sb-sub-btn" class="button" data-clipboard-text="${subSbUrl}">Copy Link</button></div><div class="client-buttons"><a href="${clashMetaUrl}" class="client-btn">Clash Meta / Stash (Sing-Box)</a><button class="client-btn" onclick="toggleQR('singbox', '${subSbUrl}')">Show QR Code</button></div><div id="qr-singbox-container" class="qr-container"><div id="qr-singbox"></div></div></div></div>
<script>
function copyToClipboard(e,t){const n=e.textContent;navigator.clipboard.writeText(t).then(()=>{e.textContent="Copied!",setTimeout(()=>{e.textContent=n},1500)})}function toggleQR(e,t){const n=document.getElementById("qr-"+e+"-container"),o=document.getElementById("qr-"+e);n.style.display&&"none"!==n.style.display?n.style.display="none":(n.style.display="block",o.hasChildNodes()||new QRCode(o,{text:t,width:256,height:256,colorDark:"#E0E0E0",colorLight:"#1E1E1E",correctLevel:QRCode.CorrectLevel.H}))}
function displayExpirationTimes(){const e=document.getElementById("expiration-display"),t=document.getElementById("expiration-relative");if(!e?.dataset.utcTime)return;const n=new Date(e.dataset.utcTime);if(isNaN(n.getTime()))return;const o=(n.getTime()-new Date().getTime())/1e3,a=o<0;if(!a&&!t.textContent.includes("Expired")&&!t.textContent.includes("Reached")){const e=new Intl.RelativeTimeFormat("en",{numeric:"auto"});let n="";n=Math.abs(o)<3600?e.format(Math.round(o/60),"minute"):Math.abs(o)<86400?e.format(Math.round(o/3600),"hour"):e.format(Math.round(o/86400),"day");t.textContent="Expires "+n}else a&&!t.textContent.includes("Expired")&&!t.textContent.includes("Reached")&&(t.textContent="Subscription Expired",t.classList.add("status-expired-text"),t.classList.remove("status-active-text"));document.getElementById("local-time").textContent=n.toLocaleString(void 0,{timeZoneName:"short"});document.getElementById("tehran-time").textContent=n.toLocaleString("en-US",{timeZone:"Asia/Tehran",hour12:!0,year:"numeric",month:"short",day:"numeric",hour:"numeric",minute:"2-digit"});document.getElementById("utc-time").textContent=n.toISOString().substring(0,19).replace("T"," ")+" UTC"}
function displayDataUsage(){const e=document.getElementById("data-usage-display"),t=parseInt(e.dataset.usage,10),n=parseInt(e.dataset.limit,10),o=e=>{if(e<=0)return"0 Bytes";const t=Math.floor(Math.log(e)/Math.log(1024));return parseFloat((e/Math.pow(1024,t)).toFixed(2))+" "+["Bytes","KB","MB","GB","TB"][t]},a=n>0?o(n):"&infin;";e.innerHTML=o(t)+" / "+a}
async function fetchNetworkInfo(){try{const e=await fetch("/network-info"),t=await e.json();document.getElementById("proxy-ip").textContent=t.proxy?.ip||"N/A";document.getElementById("proxy-location").textContent=(t.proxy?.city||"")+(t.proxy?.city&&t.proxy?.country?", ":"")+(t.proxy?.country||"N/A");document.getElementById("proxy-isp").textContent=t.proxy?.isp||"N/A";document.getElementById("user-ip").textContent=t.user?.ip||"N/A";document.getElementById("user-location").textContent=(t.user?.city||"")+(t.user?.city&&t.user?.country?", ":"")+(t.user?.country||"N/A");document.getElementById("user-isp").textContent=t.user?.isp||"N/A";document.getElementById("user-risk").textContent=t.user?.risk||"N/A"}catch(e){console.error("Network info refresh failed:",e),document.getElementById("proxy-ip").textContent="Error",document.getElementById("user-ip").textContent="Error"}}
document.addEventListener("DOMContentLoaded",()=>{displayExpirationTimes(),displayDataUsage(),fetchNetworkInfo(),document.getElementById("refresh-network-btn").addEventListener("click",()=>{document.querySelectorAll('.network-info-grid span:not([id$="-risk"])').forEach(e=>{e.innerHTML='<span class="skeleton"></span>'}),document.getElementById("user-risk").innerHTML='<span class="skeleton"></span>',fetchNetworkInfo()}),document.querySelectorAll(".button[data-clipboard-text]").forEach(e=>{e.addEventListener("click",()=>copyToClipboard(e,e.dataset.clipboardText))}),setInterval(displayExpirationTimes,6e4)});
</script>
</body></html>`;
    return html;
}
// ***** END: CORRECTED generateBeautifulConfigPage *****


// --- Core VLESS Connection Logic (from Script 1, with Script 2 features injected) ---
...
function stringify(arr, offset = 0) { 
  const uuid = unsafeStringify(arr, offset); if (!isValidUUID(uuid)) throw new TypeError('Stringified UUID is invalid'); return uuid;
}

async function ProtocolOverWSHandler(request, env, ctx, cfg) { 
  const webSocketPair = new WebSocketPair(); const [client, webSocket] = Object.values(webSocketPair); webSocket.accept(); let address = ''; let portWithRandomLog = ''; let udpStreamWriter = null;
  let sessionUsage = 0;
  let userUUID = '';
  ...
  const updateUsageInDB = async () => {
    if (sessionUsage > 0 && userUUID) {
      try {
        ...
        await env.DB.prepare("UPDATE users SET data_usage = data_usage + ? WHERE uuid = ?").bind(usage, userUUID).run();
  ...
  const createUsageCountingTransform = () => new TransformStream({
    transform(chunk, controller) {
      sessionUsage += chunk.byteLength; // Add chunk size to total session traffic
      controller.enqueue(chunk);
    }
  });

  const usageCounterDownstream = createUsageCountingTransform(); // Upload
  const usageCounterUpstream = createUsageCountingTransform(); // Download

  const requestConfig = { socks5Address: cfg.socks5.address, socks5Relay: cfg.socks5.relayMode, enableSocks: cfg.socks5.enabled, parsedSocks5Address: cfg.socks5.enabled ? socks5AddressParser(cfg.socks5.address) : {} };
  const earlyDataHeader = request.headers.get('Sec-WebSocket-Protocol') || ''; const readableWebSocketStream = MakeReadableWebSocketStream(webSocket, earlyDataHeader, log); let remoteSocketWapper = { value: null };

  readableWebSocketStream
    .pipeThrough(usageCounterDownstream) // Counts Uploads
    .pipeTo(
      new WritableStream({
        async write(chunk, controller) {
          ...
          if (hasError || !user) {
            controller.error(new Error(message || 'Invalid user.'));
            return;
          }
          ...
          if (isUDP) {
            if (portRemote === 53) {
              ...
              controller.error(new Error('UDP only for DNS (port 53)'));
            }
            return;
          }
          ...
          HandleTCPOutBound(
            ...
            log,
            requestConfig,
            usageCounterUpstream // Pass the Download counter
          );
        },
        close() {
...
async function ProcessProtocolHeader(protocolBuffer, env, ctx) { 
  ...
  const userData = await getUserData(env, slicedBufferString, ctx);
  ...
  const optLength = dataView.getUint8(17); const command = dataView.getUint8(18 + optLength); if (command !== 1 && command !== 2) return { hasError: true, message: `command ${command} not supported` }; const portIndex = 18 + optLength + 1; const portRemote = dataView.getUint16(portIndex); const addressType = dataView.getUint8(portIndex + 2); let addressValue, addressLength, addressValueIndex; switch (addressType) { case 1: addressLength = 4; addressValueIndex = portIndex + 3; addressValue = new Uint8Array(protocolBuffer.slice(addressValueIndex, addressValueIndex + addressLength)).join('.'); break; case 2: addressLength = dataView.getUint8(portIndex + 3); addressValueIndex = portIndex + 4; addressValue = new TextDecoder().decode(protocolBuffer.slice(addressValueIndex, addressValueIndex + addressLength)); break; case 3: addressLength = 16; addressValueIndex = portIndex + 3; addressValue = Array.from({ length: 8 }, (_, i) => dataView.getUint16(addressValueIndex + i * 2).toString(16)).join(':'); break; default: return { hasError: true, message: `invalid addressType: ${addressType}` }; } if (!addressValue) return { hasError: true, message: `addressValue empty, type ${addressType}` };
  
  return { user: userData, hasError: false, addressRemote: addressValue, addressType, portRemote, rawDataIndex: addressValueIndex + addressLength, ProtocolVersion: new Uint8Array([version]), isUDP: command === 2 };
}

async function HandleTCPOutBound( remoteSocket, addressType, addressRemote, portRemote, rawClientData, webSocket, protocolResponseHeader, log, config, usageCounterUpstream ) { 
  async function connectAndWrite(address, port, socks = false) { let tcpSocket; if (config.socks5Relay) { tcpSocket = await socks5Connect(addressType, address, port, log, config.parsedSocks5Address); } else { tcpSocket = socks ? await socks5Connect(addressType, address, port, log, config.parsedSocks5Address) : connect({ hostname: address, port }); } remoteSocket.value = tcpSocket; log(`connected to ${address}:${port}`); const writer = tcpSocket.writable.getWriter(); await writer.write(rawClientData); writer.releaseLock(); return tcpSocket; }
  async function retry() { const tcpSocket = config.enableSocks ? await connectAndWrite(addressRemote, portRemote, true) : await connectAndWrite(addressRemote, portRemote, false); tcpSocket.closed.catch(error => console.log('retry error', error)).finally(() => safeCloseWebSocket(webSocket)); RemoteSocketToWS(tcpSocket, webSocket, protocolResponseHeader, null, log, usageCounterUpstream); }
  const tcpSocket = await connectAndWrite(addressRemote, portRemote);
  RemoteSocketToWS(tcpSocket, webSocket, protocolResponseHeader, retry, log, usageCounterUpstream);
}

function MakeReadableWebSocketStream(webSocketServer, earlyDataHeader, log) { 
  return new ReadableStream({ start(controller) { webSocketServer.addEventListener('message', (event) => controller.enqueue(event.data)); webSocketServer.addEventListener('close', () => { safeCloseWebSocket(webSocketServer); controller.close(); }); webSocketServer.addEventListener('error', (err) => { log('ws error'); controller.error(err); }); const { earlyData, error } = base64ToArrayBuffer(earlyDataHeader); if (error) controller.error(error); else if (earlyData) controller.enqueue(earlyData); }, pull(_controller) { }, cancel(reason) { log(`ReadableStream cancelled: ${reason}`); safeCloseWebSocket(webSocketServer); }, });
}
...
async function RemoteSocketToWS(remoteSocket, webSocket, protocolResponseHeader, retry, log, usageCounterUpstream) { 
  ...
  try {
    await remoteSocket.readable
      .pipeThrough(usageCounterUpstream) // Counts Download
      .pipeTo(
        new WritableStream({
          async write(chunk) {
            if (webSocket.readyState !== CONST.WS_READY_STATE_OPEN) throw new Error('WS not open');
          ...
          close() {
            log(`Remote readable closed. Data received: ${hasIncomingData}`);
          },
          abort(reason) {
...
function base64ToArrayBuffer(base64Str) { 
  if (!base64Str) return { earlyData: null, error: null }; try { const binaryStr = atob(base64Str.replace(/-/g, '+').replace(/_/g, '/')); const buffer = new ArrayBuffer(binaryStr.length); const view = new Uint8Array(buffer); for (let i = 0; i < binaryStr.length; i++) view[i] = binaryStr.charCodeAt(i); return { earlyData: buffer, error: null }; } catch (error) { return { earlyData: null, error }; }
}
...
async function createDnsPipeline(webSocket, vlessResponseHeader, log, updateUpstreamUsage) { 
  let isHeaderSent = false; const transformStream = new TransformStream({ transform(chunk, controller) { for (let index = 0; index < chunk.byteLength;) { const lengthBuffer = chunk.slice(index, index + 2); const udpPacketLength = new DataView(lengthBuffer).getUint16(0); const udpData = new Uint8Array(chunk.slice(index + 2, index + 2 + udpPacketLength)); index = index + 2 + udpPacketLength; controller.enqueue(udpData); } }, });
...
async function socks5Connect(addressType, addressRemote, portRemote, log, parsedSocks5Addr) { 
  const { username, password, hostname, port } = parsedSocks5Addr; const socket = connect({ hostname, port }); const writer = socket.writable.getWriter(); const reader = socket.readable.getReader(); const encoder = new TextEncoder(); await writer.write(new Uint8Array([5, 2, 0, 2])); let res = (await reader.read()).value; if (res[0] !== 0x05 || res[1] === 0xff) throw new Error('SOCKS5 connection failed.'); if (res[1] === 0x02) { if (!username || !password) throw new Error('SOCKS5 auth needed.'); const authRequest = new Uint8Array([ 1, username.length, ...encoder.encode(username), password.length, ...encoder.encode(password), ]); await writer.write(authRequest); res = (await reader.read()).value; if (res[0] !== 0x01 || res[1] !== 0x00) throw new Error('SOCKS5 auth failed.'); } let DSTADDR; switch (addressType) { case 1: DSTADDR = new Uint8Array([1, ...addressRemote.split('.').map(Number)]); break; case 2: DSTADDR = new Uint8Array([3, addressRemote.length, ...encoder.encode(addressRemote)]); break; case 3: DSTADDR = new Uint8Array([ 4, ...addressRemote.split(':').flatMap((x) => [parseInt(x.slice(0, 2), 16), parseInt(x.slice(2), 16)]), ]); break; default: throw new Error(`Invalid addressType for SOCKS5: ${addressType}`); } const socksRequest = new Uint8Array([5, 1, 0, ...DSTADDR, portRemote >> 8, portRemote & 0xff]); await writer.write(socksRequest); res = (await reader.read()).value; if (res[1] !== 0x00) throw new Error('Failed SOCKS5 request.'); writer.releaseLock(); reader.releaseLock(); return socket;
}

function socks5AddressParser(address) { 
  try { const [authPart, hostPart] = address.includes('@') ? address.split('@') : [null, address]; const [hostname, portStr] = hostPart.split(':'); const port = parseInt(portStr, 10); if (!hostname || isNaN(port)) throw new Error(); let username, password; if (authPart) { [username, password] = authPart.split(':'); if (!username) throw new Error(); } return { username, password, hostname, port }; } catch { throw new Error('Invalid SOCKS5 address. Expected [user:pass@]host:port'); }
}

// --- Main Fetch Handler (Router - Final Version) ---
...
export default {
  async fetch(request, env, ctx) {
    ...
    // 3. WebSocket/VLESS (Stable Core + Traffic Count)
    const upgradeHeader = request.headers.get('Upgrade');
    if (upgradeHeader?.toLowerCase() === 'websocket') {
      if (!env.DB || !env.USER_KV) {
        return new Response(JSON.stringify({ error: 'VLESS proxy misconfigured.' }), { status: 503, headers: { 'Content-Type': 'application/json' } });
      }
    ...
    const handleSubscription = async (core) => {
      ...
      // Check user validity, expiration, and data limit before serving subscription
    ...
    if (isValidUUID(path)) {
      ...
      // Note: We show the config page even if expired or data limit reached,
      // so the user can see their status.
    ...
    if (cfg.rootProxyUrl) {
      try {
        ...
        targetUrl.hostname = proxyUrl.hostname; targetUrl.protocol = proxyUrl.protocol; targetUrl.port = proxyUrl.port;
        ...
        mutableHeaders.delete('Content-Security-Policy'); mutableHeaders.delete('X-Frame-Options');
